...
def score_segment(start,end,x,y):
    ...
    score = (np.abs(max-mean))*np.power((end-start),0.5)
    return score
...
# Calculate piecewise constant approximation   #
################################################
def calculate_piecewise_constant_approx_new(x,y):
    ...
    while(fxn_gain>gain_threshold):
        ...
        for ii in np.arange(0,1,0.01):
            for jj in np.arange(ii,1,0.01):
                if ii not in support_nodes and jj not in support_nodes:
                    support_nodes_temp = np.sort(np.append(np.append(support_nodes,ii),jj))
                    temp_score = score_set_of_supports(x,y,support_nodes_temp)
        ...
        print("BEST NEW LEVEL: ",best_node_start,best_node_end," ",last_score-best_score)
        fxn_gain = last_score-best_score
        if(fxn_gain>gain_threshold):
            support_nodes = np.sort(np.append(np.append(support_nodes, best_node_start),best_node_end))
...
def main():
    ...
    support_nodes = calculate_piecewise_constant_approx_new(x,y)
